...
Laptop docked, lid closed, LG UltraGear 38GN950 at 3840x1600 on the RTX 3060.
After 3-5 hours of ordinary uptime — no sleep, no dock cycle — everything
composited starts to judder: windows, scrolling, CAD crosshairs. The hardware
mouse cursor stays perfectly smooth. Dragging a window buys 1-5 seconds of
relief, then it returns. Cycling the Intel UHD adapter cures it instantly
(Ctrl+Alt+D on this machine). Nothing else does: not restarting `dwm.exe`, not
the driver reset, not resetting either NVIDIA container, not closing apps, not
a UAC secure-desktop switch, not a monitor power cycle.

We have a cure. We do not have a root cause. This kit exists to get one.
