...
- **Cause:** when the external monitor disappears, Windows switches output to the **laptop's own internal panel**
  ...
  (DWM) stalls on it: fixed-length freezes on a fixed rhythm, with roughly **half of all frames never reaching the
  screen**.
- **Instant fix (no reboot, no downloads):** Device Manager, *Display adapters*, **Intel(R) UHD Graphics**,
  **Disable**, wait 3 seconds, **Enable**. The screen blinks for 1-2 s and the stutter is gone.
  Or use the script or one-key hotkey in [`fix/`](fix/).
- **How sure is this?** Reproduced **on demand** (turn the monitor off and on, get the stutter). Removing the ghost
  cures it (**4 for 4**). Keeping the Intel GPU disabled **prevents it entirely** (2 for 2). Frame-timing captures of
...
After years of chasing this on a laptop I bought (reboots, driver changes, forum threads that went nowhere), it was
finally tracked down with the help of **Claude**, by instrumenting the machine properly instead of guessing:
...
`RESULT: GHOST` means the Intel GPU is driving a display that is not on your desktop, which is the state this repo
fixes. In the healthy state it reports `CLEAN`.
...
2. Right-click **Intel(R) UHD Graphics** (or Iris Xe), choose **Disable device**, confirm.
3. Wait about 3 seconds. The screen may blink.
...
3. You come back. The monitor wakes up and Windows puts your desktop back on it. **But it never switches the internal
   panel off again.** The Intel chip keeps driving a dark, invisible 1920x1080 @ 144 Hz display.
4. The **Desktop Window Manager**, the part of Windows that draws everything you see, is now also tied to that ghost
   display. It keeps **freezing for a fixed length of time on a fixed rhythm** (on the test machine, 60-80 ms freezes
   every 120-140 ms), throwing away about half of every frame your apps draw.
5. The mouse pointer is drawn by the monitor hardware itself, not by DWM, which is why **it** stays smooth.

Disabling the Intel adapter destroys that ghost display path. When it is re-enabled it comes back with **no** display
attached, which is the healthy state, and DWM runs smoothly again.
...
| UAC / secure-desktop switch, Ctrl+Alt+Del then Cancel, lock + unlock | No |
...
| Reboot, or a restart attempt cancelled at the "apps are preventing restart" screen | Yes |
...
3. Windows reports the internal panel as the **only** display (e.g. `\\.\DISPLAY20 1920x1080@144`). The Intel adapter
   goes to **Availability 3, 1920x1080@144**.
...
6. `Disable-PnpDevice` on the Intel adapter clears the stutter at the disable (before the re-enable).
   With the Intel adapter held disabled, steps 2-4 produce a virtual `640x480@64` display instead, dropped cleanly
   on monitor return, and **no stutter**.
...
| Stall length / period | 62-70 ms / every 120 ms | 76-83 ms / every 141 ms | none |
...
The application keeps presenting at about 144 fps (median 6.9 ms) in every state. **DWM** is what stalls, with a
**fixed stall length on a fixed period** (1 ms jitter around the period), which is the signature of a wait that times
out rather than of load. Stall length differs between occurrences (62.5 vs 76 ms), so it does not look like a single
constant timeout. One untested idea: the two numbers behave like a phase relationship between two display clocks, the
external monitor and the dark internal panel.
...
| External | LG UltraGear 38GN950, 3840x1600 @ 144 Hz, through an HP Thunderbolt Dock G4 on the laptop's USB-C port (no Thunderbolt controller on this laptop, so DP Alt Mode). Wired to the NVIDIA GPU: it stays up with the Intel adapter disabled |
| Internal panel | 1920x1080 @ 144 Hz |
...
pressure (1.6 of 6 GB), display mode or link drop (external stays 3840x1600 @ 143), CPU load and DPC storms (DPC
...
Diagnosed by the owner of the affected laptop together with **Claude**, September 2026.
